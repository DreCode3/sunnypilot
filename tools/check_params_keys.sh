...
# Exclude test files: they intentionally use throwaway/dummy param keys (e.g. upstream's
# test_params.py "swag" fixture) that are NOT — and should not be — registered in params_keys.h.
STAGED=$(git diff --cached --name-only --diff-filter=ACMR | grep '\.py$' \
  | grep -vE '(^|/)test_[^/]*\.py$|/tests?/|(^|/)conftest\.py$' || true)
